...
namespace MS_Sql
{
    public partial class Form1 : Form
    {
        ...
        string connectionString = @"Server=192.168.56.101;port=3306; database=BeautySalon; user=root; password=123456; Character Set=utf8;";
        public Form1()
        {
            InitializeComponent();
        }
        ...
        public void GetQuery(string com)
        {
            ...
            {
                con.Open();
                MySqlDataAdapter ad = new MySqlDataAdapter(command, connectionString);
                ad.Fill(data);
            }
        }
        public void CheckStart(string select, string insert) //перевірка наявності початкових рядків (значень) у таблиці
        {
            ...
            {
                con.Open();
                MySqlDataAdapter ad = new MySqlDataAdapter(select, connectionString);
                ad.Fill(data);
            }
            try
            {
        ...
        public void FillTypes() //заповнення випадаючого списку типів
        {
            ...
            {
                con.Open();
                MySqlDataAdapter ad = new MySqlDataAdapter("SELECT Category_Name From Category", connectionString);
                ad.Fill(data);
            }
        ...
        private void buttonQUERY_Click(object sender, EventArgs e)
        {
            ...
            {
                res = "1 row(s) deleted";
                query = "delete from beauty where ID =" + textBoxID.Text;
            }
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(connectionString))
                {
                    ...
                    {
                        adapt = new MySqlDataAdapter("select b.ID, b.service, b.price, b.master, b.duration, b.categoryid, c.Category_Name, c.Description from beauty as b join Category as c on b.categoryid = c.ID;", connectionString);
                        data = new DataSet();
                        adapt.Fill(data);
                        dataGridView1.DataSource = data.Tables[0];
                    }
